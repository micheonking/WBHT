...
public class GridBuilder<T> {
	...
	public void setRowNumHidden(boolean hidden) {
		this.rowNum.setHidden(hidden);
	}
	...
	public Grid<T> getGrid(){
		...
		this.build(); // 일단 먼저 불러본다. 이후에 분리하는 작업이 필요하다. 
		...
		// 데이터 변경 시 처리해야 할 이벤트를 받아서 적용한다.
		if(this.beforeStartEditHandler != null) {
			editing.addBeforeStartEditHandler(this.beforeStartEditHandler);
		}
...
//					
//					new SimpleMessage(editingCell.getCol() + "");
//				}
//			  });
...
//					GridCell editingCell = event.getEditCell();			
//					Info.display("cellName", editingCell.toString());
//				}
//			  });
	...
	public ColumnConfig<T, Double> addDouble(ValueProvider<T, Double> valueProvider, int width, String header){
		...
		column.setColumnStyle(SafeStylesUtils.fromTrustedString("text-align:right;"));
...
//	public ColumnConfig<T, Boolean> addBoolean(ValueProvider<T, Boolean> valueProvider, int width, String header, CheckBox checkBox){
//		ColumnConfig<T, Boolean> column = this.addBoolean(valueProvider, width, header); 
//		editorMap.put(column, checkBox);
//		return column; 
//	}  
	...
	public ColumnConfig<T, String> addCell(ValueProvider<T, String> valueProvider, int width, String header, Cell<String> cell){
		 ...
		 SafeHtml safeEscapedHtml = SafeHtmlUtils.fromTrustedString(header); 

		 // user type cell 
		ColumnConfig<T, String> column = new ColumnConfig<T, String>(valueProvider, width, safeEscapedHtml) ;
		column.setCell(cell);
